...
This is a little helper class I created while going through [these](https://learnopengl.com/) tutorials. Most of the OpenGL tutorials I came across implement a first-person shooter (FPS) style camera system for viewport navigation, which I find less flexible compared to an orbit camera system used in most 3D authoring software.

## The Code
The camera object expects two floats as inputs (I recommend using `delta x` and `delta y` cursor coordinates) and enables tumbling, tracking and dollying. The object outputs view and projection matrices for use in your shader.
